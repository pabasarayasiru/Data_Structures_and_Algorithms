...
struct HashTable{
	...
	//add your code below
        for (char letter:user_name){
            int val_of_letterS= static_cast<int>(letter);     // get intiger values of strings
            sum+=val_of_letterS;
        }
	...
	//add an if condition to complete the code here
        if  (empty){
            password[hash]=user_password;    // insert password to the slot
    ...
    void hash_lookup(string user_name){
        ...
        empty = is_slot_empty(hash);
	//add an if condition to complete the code here
        if (empty)     // check if the password exists
        {
            cout<< "Password Not Found"<< endl;
        }
        else{
            cout<< user_name << " Password is "<< password[hash]<<endl;
        }
